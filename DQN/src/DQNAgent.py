...
class Agent():
    def __init__(self, state_size, action_size, stack_size, memory_size, batch_size, explore_stop, decay_rate, gamma, learning_rate, tau, update_every):
        ...
        self.dqn = Network(state_size, action_size, learning_rate, 'local')
        self.target_dqn = Network(state_size, action_size, learning_rate, 'target')
    ...
    def step(self, state, action, reward, next_state, done):
        self.memory.add((state.reshape(*self.state_size), action, reward, next_state, done))
    ...
    def act(self, state, isTraining):
        if isTraining is False:
            if self.explore_stop > np.random.rand():
    ...
    def preprocess(self, frame):
        return resize(rgb2gray(frame)[30:-10], [84, 84]).reshape([84, 84])
    
    def stack_frames(self, frame, is_new):
